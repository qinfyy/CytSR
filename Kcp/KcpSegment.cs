...
namespace System.Net.Sockets.Kcp
{
    ...
    public struct KcpSegment : IKcpSegment
    {
...
#endif
#if KCP_PERFORMANCE_TEST
            Console.WriteLine($"[KcpSegment] KcpSegment memory alloc: 0x{intPtr:x}");
#endif
            unsafe
            {
                ///清零    不知道是不是有更快的清0方法？
...
#endif
#if KCP_PERFORMANCE_TEST
                Console.WriteLine($"[KcpSegment] KcpSegment memory free: 0x{(IntPtr)seg.ptr:x}");
#endif
            }
        }
        ...
        public byte frg
        {
            ...
            {
#if KCP_PERFORMANCE_TEST
                Console.WriteLine($"[KcpSegment] Invoker: [{Invoker()}] assigned frg:{value} for KcpSegment: {this.ToLogString(false)}");
#endif
                unsafe
                {
                    // miHoMo KCP modify: IUINT32 token
        ...
        public uint sn
        {
            ...
            {
#if KCP_PERFORMANCE_TEST
                Console.WriteLine($"[KcpSegment] Invoker: [{Invoker()}] assigned sn:{value} for KcpSegment: {this.ToLogString(false)}");
#endif
                unsafe
                {
                    // miHoMo KCP modify: IUINT32 token
