        ...
        public virtual int Input(byte[] buffer)
        {
            switch (_State)
            {
                ...
                case ConnectionState.CONNECTED:
                    {
                        //Log.Debug($"ConnectedNotify, handle = {ikcpHandle}, buf = {Convert.ToHexString(buffer)}");
                        if (buffer.Length == 20) // Possibly a "disconnect" packet
                        {
                            ...
                            try
                            {
                                ...
                                Log.Debug($"DisconnectedNotify, Conv={disconn.Conv}, Token={disconn.Token}, Data={disconn.Data}");
                                return 0;
                            }
                            catch (ArgumentException)
                            {
                                // Do nothing
                                Log.Debug($"ConnectedNotify: Packet length=20, content={Convert.ToHexString(buffer)}");
                            }
                        }
...
#pragma warning restore CS8602 // 解引用可能出现空引用。
                        if (status != 0)
                            Log.Debug($"ikcp_input failed, return {status}, received {buffer.Length} bytes");
                        // if (status == -1)
                        // {
                        //     _State = ConnectionState.CLOSED;
                ...
                case ConnectionState.HANDSHAKE_WAIT:
                    {
                        ...
                        try
                        {
                            // Log.Debug($"HandShakeWaitNotify, buf = {Convert.ToHexString(buffer)}");
...
#pragma warning restore CS8602 // 解引用可能出现空引用。
                        ...
                        {
                            Log.Debug($"HandShakeWaitNotify: handshake fail, content={Convert.ToHexString(buffer)}");
                ...
                case ConnectionState.HANDSHAKE_CONNECT:
                    {
                        ...
                        try
                        {
                            // Log.Debug($"HandShakeConnectNotify, buf = {Convert.ToHexString(buffer)}");
            ...
            return 0;
                        ...
                        {
                            Log.Debug($"HandShakeConnectNotify: handshake fail, content={Convert.ToHexString(buffer)}");
                            throw new SocketException(10053);
                        }
                    }
            }
        ...
        public int Send(ReadOnlySpan<byte> buffer)
        {
            ...
            Log.Debug($"MhyKcpBase try to send {buffer.Length} bytes (conv:{_Conv}, token:{_Token})");
#endif
...
#pragma warning restore CS8602 // 解引用可能出现空引用。
            ...
            if (ret < 0)
            {
                Log.Warn($"MhyKcpBase send attempt failure, ret: {ret}");
            }
            return ret;
        }
        ...
        private byte[]? ReceiveNonblock()
        {
            ...
            else
            {
                ...
                Log.Error($"ReceiveNonBlock should be invoked STA! invoker: {invoker}");
            }
#endif
...
#endif
#if KCP_EXPORT_PACKET_RECORD
            Log.Debug($"MhyKcpBase received {trueSize} bytes (conv:{_Conv}, token:{_Token})");
#endif
            if (trueSize != size) Log.Error($"Unexpected state: reported peek {size} bytes, ikcp_recv returns {trueSize} bytes");
            ...
            try
            {
                while (true)
                {
                    ...
                    if (checkTime_refresh || DateTimeOffset.UtcNow >= update_next_time)
                    {
                        ...
                        //Log.Debug($"ikcp handle WaitSnd: {cskcpHandle.WaitSnd}");
...
#pragma warning restore CS0162 // 检测到无法访问的代码
            ...
            {
                Log.Error($"MhyKcpBase Background Check/Update exception and restart: " + ex);
