        ...
        private static void BackgroundUpdate()
        {
            while (true)
            {
                try
                {
                    ...
                    sb.Append("\n-----------------------------------------\n");
                    #endregion
                    Log.Warn($"Detected <color=Red>invalid kcp packet</color> sent at {res.sentTime}, bytes_sent={pktContent.Length}");
                    Log.Debug(sb.ToString());
                }
                catch (Exception ex)
                {
                    Log.Error($"Error occured when handling audit kcp packet: " + ex);
                }
            }
        }
